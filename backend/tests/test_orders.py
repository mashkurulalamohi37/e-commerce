...
class TestPaymentAuthorisation:
    ...
    async def test_successful_payment_decrements_stock(self, client, product):
        ...
        assert await stock_of(product.id) == before - 3

    async def test_payment_is_idempotent(self, client, product):
        """A retried confirmation must not decrement stock twice."""
...
class TestConcurrency:
    async def test_last_unit_cannot_be_sold_twice(self, client, product):
        ...
        first = (await client.post("/api/v1/orders/", json=order_payload(product.id, qty=2))).json()
        second = (await client.post("/api/v1/orders/", json=order_payload(product.id, qty=2))).json()

        results = await asyncio.gather(
            client.post(f"/api/v1/orders/{first['id']}/pay", json={"phone": CUSTOMER_PHONE}),
            client.post(f"/api/v1/orders/{second['id']}/pay", json={"phone": CUSTOMER_PHONE}),
        )
